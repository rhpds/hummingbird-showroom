...
if ! podman images --format "{{.Repository}}:{{.Tag}}" | grep -q "^localhost/hummingbird-demo:v1$"; then
    ...
    echo "   Module 01-03 requires the output from Module 01-02"
...
if [ ! -d ~/scanning ]; then
    mkdir -p ~/scanning || {
        echo "❌ ERROR: Cannot create ~/scanning directory"
        exit 1
    }
fi
...
systemctl --user enable --now podman.socket || {
    echo "❌ ERROR: Failed to enable podman socket"
    echo "   This is required for grype and syft to access podman"
    exit 2
}
...
cosign download sbom ${HUMMINGBIRD_REGISTRY}/openjdk:21-runtime > rh-openjdk-sbom.json 2>&1 || {
    echo "❌ ERROR: Failed to download Red Hat SBOM"
    echo "   This may indicate:"
    echo "   - Registry connectivity issues"
    echo "   - cosign not working properly"
    echo "   - SBOM not available for this image"
    exit 2
}
...
grype hummingbird-demo:v1 2>&1 | tee grype-scan-raw.log | sed 's/\x1b\[[0-9;]*m//g' > grype-scan.log || {
    ...
    if [ $GRYPE_EXIT -gt 1 ]; then
        echo "❌ ERROR: Grype scan failed with exit code $GRYPE_EXIT"
        cat grype-scan.log
        exit 2
    fi
}
...
syft hummingbird-demo:v1 -o table > sbom-table.txt 2>&1 || {
    echo "❌ ERROR: SBOM table generation failed"
    cat sbom-table.txt
    exit 2
}
...
if [ -z "$PACKAGE_COUNT" ]; then
    echo "❌ ERROR: Cannot extract package count from SBOM"
    exit 3
fi
...
if [ -z "$SBOM_NAME" ]; then
    echo "❌ ERROR: SBOM missing 'name' field"
    exit 3
fi
...
if [ ! -f rh-openjdk-sbom.json ] || [ ! -s rh-openjdk-sbom.json ]; then
    echo "❌ ERROR: Red Hat SBOM file is missing or empty"
    exit 3
fi
...
if podman images --format "{{.Repository}}:{{.Tag}}" | grep -q "^localhost/hummingbird-demo:ubi$"; then
    ...
    grep -A 2 "Scanned for vulnerabilities" grype-ubi-scan.log || echo "UBI scan completed"
