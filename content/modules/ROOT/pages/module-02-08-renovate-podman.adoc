...
cat > renovate.json << 'RENOVATE_EOF'
{
  ...
  "schedule": ["after 6am and before 9am on weekdays"],
...
=== Step 1: Create the GitHub Token Secret

Renovate needs a GitHub Personal Access Token with `repo`, `read:org`, and `read:packages` scopes.
...
[IMPORTANT]
====
Replace `ghp_yourTokenHere` with your actual GitHub PAT. Create one at https://github.com/settings/tokens with scopes: `repo`, `read:org`, `read:packages`.

For production environments, use a GitHub App installation token instead of a PAT for better security and rate limits.
====
...
1. **Replace placeholder tokens** in the Secret and ConfigMap with your actual GitHub PAT and repository list
2. **Configure the GitHub webhook** to point at your EventListener ingress URL
3. **Customize `renovate.json`** in each repo to match your specific dependency patterns
4. **Add Slack/Teams notifications** using Tekton Results or a notification Task at the end of the pipeline
5. **Integrate with ACS** (xref:module-02-07-acs-zero-cve.adoc[Sub-Module 2.7]) to enforce policies on Renovate-built images
