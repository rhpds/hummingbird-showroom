...
* Red Hat Quay operator installed (on-cluster registry with Clair)
...
The script applies Kustomize overlays in order, waits for each component to become ready, then runs post-configuration (Quay user initialization, registry credentials, ACS init-bundle generation, `roxctl` install). This is equivalent to running all of Option 3's manual steps.
...
Install all four operators (OpenShift Pipelines, Builds/Shipwright, Quay, ACS) using the Kustomize overlays from the workshop repository.
...
[NOTE]
====
This single command installs four operators:
...
The Kustomize overlays reference the https://github.com/tosin2013/gitops-catalog[gitops-catalog] for Pipelines and ACS, and use local manifests for Builds and Quay.
====
...
==== Step 10: Create Quay Configuration Bundle
...
==== Step 11: Deploy QuayRegistry Instance
