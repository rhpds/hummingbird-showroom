...
=== Step 2: Deploy QuayRegistry Instance

Create a QuayRegistry custom resource. The `managed` configuration lets the operator handle all dependencies (PostgreSQL, Redis, object storage, Clair) automatically:
...
=== Step 3: Get the Quay Registry Route
...
=== Step 4: Create a Workshop User and Organisation
...
=== Step 5: Create Registry Credentials Secret
...
=== Step 6: Link Secret to Pipeline ServiceAccount
...
=== Step 5: Deploy the Secured Cluster Services

The Secured Cluster bundle connects your OpenShift cluster to ACS Central for runtime monitoring and admission control:

. In ACS Central, navigate to *Platform Configuration* -> *Clusters*
. Click *New Cluster*
. Name: `workshop-cluster`
. Set the Central API Endpoint to `central.stackrox.svc:443`
. Click *Next* and download the YAML bundle
. Apply the bundle:

[source,bash,role=execute]
----
oc apply -R -f sensor-bundle/
----

Alternatively, create the SecuredCluster CR directly:
...
=== Step 7: Verify ACS Installation
