...
* `{user}-hummingbird-builds` namespace created
...
----
oc login {openshift_api_server_url} -u {openshift-user}
oc version
...
[source,bash,role=execute,subs=attributes]
----
oc new-project {user}-hummingbird-builds
----

.*Expected output:*
----
Now using project "{user}-hummingbird-builds" on server "https://...".
----
...
[source,bash,role=execute,subs=attributes]
----
oc get buildstrategy -n {user}-hummingbird-builds
----
...
[source,bash,role=execute,subs=attributes]
----
oc secrets link pipeline registry-credentials --for=pull,mount -n {user}-hummingbird-builds
----
...
Your ACS installation is ready. Proceed to Step 24 below to provision per-user ACS access before running the workshop.

==== Step 24: Provision Per-User ACS Access

Run `setup-workshop-users.sh` to generate per-user ACS API tokens and inject them into each student's Showroom environment. This step must be completed **after** ACS Central is running.

The script (one run covers all users):

* Generates an ACS API token with the **Admin role** (ACS RBAC only — no OpenShift cluster-admin) for each student account (`workshop-{username}`), enabling Exercise 7 policy creation.
* Injects `rhacs_route` and `rhacs_api_token` into each student's Showroom `user_data.yml` so the module can use them as `{rhacs_route}` and `{rhacs_api_token}` attributes.
* Pre-creates the `{username}-hummingbird-acs-lab` namespace with an `admin` RoleBinding so students can use `oc project` without cluster-admin.
* Builds a custom Showroom terminal image with `roxctl` pre-installed and pushes it to the cluster's internal image registry.

[source,bash,role=execute]
----
cd /path/to/zero-cve-hummingbird-showroom
bash scripts/setup-workshop-users.sh
----

[NOTE]
====
If ACS Central was not yet deployed when you first ran `setup-workshop-users.sh`, re-run the script after ACS is ready. The token generation and userdata injection steps are fully idempotent — existing tokens are revoked and replaced on each run.
====

Proceed to xref:module-02-07-acs-zero-cve.adoc[Sub-Module 2.2: The Immutable Fortress].
...
==== Step 25: Install the RHTAS Operator
...
==== Step 26: Create the Keycloak Realm for RHTAS
...
[[rhtas-cli-tools]]
==== Step 27: Download RHTAS CLI Tools
