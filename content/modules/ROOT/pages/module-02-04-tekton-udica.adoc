...
[source,bash,role=execute]
----
oc get pipeline selinux-policy-pipeline -n hummingbird-builds
----
...
[source,bash,role=execute]
----
oc get pipelinerun generate-nodejs-policy -n hummingbird-builds -w
----
...
[source,bash,role=execute]
----
TASKRUN=$(oc get pipelinerun generate-nodejs-policy -n hummingbird-builds \
  -o jsonpath='{.status.childReferences[0].name}')
oc get taskrun $TASKRUN -n hummingbird-builds \
...
[source,bash,role=execute]
----
oc logs ${TASKRUN}-pod -n hummingbird-builds -c step-generate-policy
----
...
[source,bash,role=execute]
----
oc get pods -l tekton.dev/pipelineRun=generate-nodejs-policy -n hummingbird-builds
----
====
...
Check the events to see the specific error:

[source,bash,role=execute]
----
oc describe pod -l app=nodejs-selinux -n hummingbird-builds | grep -A2 "Warning"
----

.*Expected output:*
----
Warning  Failed  ...  kubelet  Error: container create failed: write to `/proc/self/attr/keycreate`: Invalid argument
----

[IMPORTANT]
====
**The `CreateContainerError` is expected in a workshop environment.** The error `write to /proc/self/attr/keycreate: Invalid argument` means the SELinux type `hummingbird_nodejs.process` does not exist on the cluster nodes. The CIL policy was generated by the pipeline but has not been loaded via MachineConfig (which triggers a rolling node reboot).

The MachineConfig section below shows how production clusters handle this. You can clean up the failing deployment:

[source,bash,role=execute]
----
oc delete deployment nodejs-selinux -n hummingbird-builds
----
====
...
[TIP]
====
A working reference implementation of this structure is available at https://github.com/tosin2013/sample-nodejs-hummingbird[sample-nodejs-hummingbird^]. It includes the Tekton Task, Pipeline, PipelineRun template, Deployment, MachineConfig, and generated CIL policy from this sub-module.
====
...
* https://github.com/tosin2013/sample-nodejs-hummingbird[sample-nodejs-hummingbird^] -- Node.js application with SELinux CI/CD manifests (Tekton, Deployment, MachineConfig, CIL policy)
