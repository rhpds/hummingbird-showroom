...
spec:
  ...
  strategy:
    ...
    kind: ClusterBuildStrategy
  output:
    image: quay.io/{registry-user}/auto-detect-app:latest
    credentials:
...
[IMPORTANT]
====
Replace `{registry-user}` with your actual registry username/organization.
====
...
* **External registries** (quay.io, Docker Hub): Require credentials
* **Internal registry**: No credentials within same cluster
