...
----
oc get buildrun auto-detect-build-run-1 -n hummingbird-builds-{user} -w
----
...
[source,bash,role=execute,subs=attributes]
----
BUILDRUN_POD=$(oc get buildrun auto-detect-build-run-1 -n hummingbird-builds-{user} -o jsonpath='{.status.taskRunName}')-pod

# View detect-and-generate step output
oc logs $BUILDRUN_POD -n hummingbird-builds-{user} -c step-detect-and-generate
----
...
[source,bash,role=execute,subs=attributes]
----
oc logs $BUILDRUN_POD -n hummingbird-builds-{user} -c step-build-and-push | head -20
----
...
spec:
  ...
  strategy:
    ...
    kind: ClusterBuildStrategy
  output:
    image: image-registry.openshift-image-registry.svc:5000/hummingbird-builds-{user}/sample-app:latest
EOF
----
...
[source,bash,role=execute,subs=attributes]
----
oc get imagestream sample-app -n hummingbird-builds-{user}
oc get imagestream sample-app -n hummingbird-builds-{user} -o jsonpath='{.status.tags[0].items[0].dockerImageReference}'
echo ""
----
...
sample-app   image-registry.openshift-image-registry.svc:5000/hummingbird-builds-{user}/sample-app   latest   ...
----
