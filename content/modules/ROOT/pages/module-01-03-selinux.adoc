...
* Installng custom CA bundles in images
...
podman build -t caddy:ssl -f ~/webserver/Containerfile
...
[source,bash,role=execute]
----
podman run --net=host --rm -it  quay.io/hummingbird-hatchling/curl https://localhost:8443
----
...
The `fips` variant changes the algorithms available in the container and also uses the validated FIPS proivider delivered in RHEL.  Note that while the algorithms are always enforced, the underlying host or cluster must also be operating in a FIPS mode to ensure full compliance.

In this lab, the host and cluster are *not* in FIPS mode, howver the container restrictions will be enforced regardless. A `0` in the `fips_enabled` parameter means the kernel boot parameter was not set.
...
[source,bash,role=execute]
----
podman build -t fips:no -f ~/fips/Containerfile 
podman build -t fips:yes -f ~/fips/Containerfile.fips 
----
