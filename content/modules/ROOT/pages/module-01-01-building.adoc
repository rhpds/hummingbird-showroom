...
Let's start by examining a professional HTML landing page that has been pre-created to demonstrate web containerization:
...
Key web technologies demonstrated:
* **Tailwind CSS**: Modern utility-first CSS framework loaded from CDN
* **Google Fonts**: Professional typography with Inter font family
* **Responsive Design**: Mobile-first approach with responsive breakpoints  
* **Modern HTML5**: Semantic structure with accessibility considerations

This demonstrates modern web development patterns commonly seen in containerized applications.
...
A Flask web application has been pre-created for containerization. Let's examine its structure:
...
Flask architecture patterns demonstrated:
* **Route Decorators**: URL mapping with `@app.route("/")` for the root path
* **Static File Serving**: HTML delivery via `send_static_file()` method
* **Development Server**: Built-in Flask server for testing and development
* **Container Configuration**: Host binding on `0.0.0.0` for container access on port 8080

The application uses the development server to serve an HTML file as a static file, which means it will be added to the `static` directory in our Containerfile later. The `index.html` file has already been copied from the webserver example.
...
[NOTE]
====
A UBI comparison Containerfile has been pre-created. Let's examine the traditional approach:
====
...
UBI characteristics vs. hardened images:
* **Full Distribution**: Complete package manager (`dnf`) and shell access
* **User Management**: Manual user/group creation required (`groupadd`, `useradd`)
* **Package Installation**: Traditional `dnf install` approach for dependencies
* **Size Impact**: Larger attack surface and image size due to complete OS
* **Flexibility**: More tools available but increased complexity
...
A production-ready multi-stage Containerfile demonstrates advanced container patterns. Let's examine its structure:
...
[source,bash,role=execute]
----
grep -E "(FROM|COPY --from)" ~/sample-app/Containerfile
----
...
* **Builder Stage**: Uses `openjdk:21-builder` with full JDK + Maven for compilation
* **Runtime Stage**: Uses `openjdk:21-runtime` with minimal JRE for deployment
* **Layer Optimization**: Dependencies copied separately from source for efficient rebuilds
* **Security Patterns**: Non-root user (65532), proper file ownership with `--chown`
* **Quarkus Fast-JAR**: Optimized startup performance layout with separated `lib/`, `app/`, `quarkus/` directories
* **Build Tools**: Includes `unzip` installation for Maven wrapper functionality
* **Environment Configuration**: JVM tuning for container deployment
...
----
[1/2] STEP 1/9: FROM {ubi-registry}/openjdk-21:latest AS builder
