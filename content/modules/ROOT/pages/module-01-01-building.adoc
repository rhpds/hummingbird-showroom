...
COPY --from=builder --chown=65532:65532 /build/target/quarkus-app/lib/ ./lib/ #<3>
...
<1> **Builder Stage**: Uses `openjdk:21-builder` with full JDK + Maven for compilation
<1> **Runtime Stage**: Uses `openjdk:21-runtime` with minimal JRE for deployment
