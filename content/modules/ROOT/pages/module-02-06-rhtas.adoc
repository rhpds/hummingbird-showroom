...
* A workstation with the `oc` binary installed and authenticated with `cluster-admin` privileges
* An OIDC provider configured (Red Hat build of Keycloak, Google, Amazon STS, Microsoft Entra ID, or GitHub)
* `cosign` version 2.2 or later (will be downloaded from the RHTAS operator after installation)
...
=== Step 1: Install the RHTAS Operator via CLI

Install the RHTAS operator using the Operator Lifecycle Manager (OLM) CLI approach:
...
[IMPORTANT]
====
The RHTAS operator is installed in the `trusted-artifact-signer` namespace. All Sigstore components (Fulcio, Rekor, CTlog, TUF) will deploy into this namespace.
====

=== Step 2: Download the CLI Tools

After the operator is installed, download the `cosign`, `rekor-cli`, and `ec` binaries. The RHTAS operator exposes these tools via a downloads route:

[source,bash,role=execute]
----
# Get the downloads route
DOWNLOADS_URL=$(oc get route -n trusted-artifact-signer -l app=trusted-artifact-signer-clientserver -o jsonpath='{.items[0].spec.host}')
...
[NOTE]
====
For RHTAS, you must use `cosign` version 2.2 or later. The binaries served by the RHTAS operator are always the correct version for your installation. If the download route is not yet available, wait 1-2 minutes for the operator to complete initialization.
====

=== Step 3: Deploy the Securesign Stack

Deploy the full Sigstore stack by creating a Securesign custom resource. The Securesign CR configures Fulcio with two OIDC issuers: one for human users (Keycloak/RHBK) and one for Kubernetes service accounts (enabling keyless pipeline signing). Under the `spec.fulcio.config.OIDCIssuers` section, edit the issuer URLs and `ClientID` for your environment:
