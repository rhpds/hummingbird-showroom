...
----
oc get buildrun secure-build-run-1 -n hummingbird-builds -w
----
...
[source,bash,role=execute]
----
TASKRUN=$(oc get buildrun secure-build-run-1 -n hummingbird-builds -o jsonpath='{.status.taskRunName}')
oc logs ${TASKRUN}-pod -n hummingbird-builds -c step-generate-sbom
----
...
[source,bash,role=execute]
----
oc logs ${TASKRUN}-pod -n hummingbird-builds -c step-scan-vulnerabilities
----
...
----
skopeo inspect --tls-verify=false --creds {registry-user}:{registry-password} \
  docker://{workshop-registry}/{registry-user}/secure-nodejs:latest \
  | python3 -c "
import json,sys
...
You can also verify the results visually in the Quay console. Navigate to **Repositories** -> **workshopuser/secure-nodejs**:
...
BUILDRUN_DIGEST=$(oc get buildrun secure-build-run-1 -n hummingbird-builds -o jsonpath='{.status.output.digest}' 2>/dev/null || echo "N/A")
