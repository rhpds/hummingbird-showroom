...
Query the Clair results via the Quay API:

[source,bash,role=execute,subs="attributes"]
----
DIGEST=$(oc get buildrun secure-build-run-1 -o jsonpath='{.status.output.digest}')
QUAY_ROUTE={workshop-registry}

curl -sk "https://${QUAY_ROUTE}/api/v1/repository/{registry-user}/secure-nodejs/manifest/${DIGEST}/security?vulnerabilities=true" | \
  python3 -c "
import json, sys
data = json.load(sys.stdin)
status = data.get('status', 'unknown')
print(f'Clair scan status: {status}')
if status == 'scanned':
    features = data.get('data', {}).get('Layer', {}).get('Features', [])
    total_vulns = sum(len(f.get('Vulnerabilities', [])) for f in features)
    print(f'Packages scanned: {len(features)}')
    print(f'Total vulnerabilities: {total_vulns}')
else:
    print('Scan may still be in progress -- check the Quay console.')
"
----

.*Expected output for a Hummingbird image:*
----
Clair scan status: scanned
Packages scanned: 15
Total vulnerabilities: 0
----

Query Clair scan results via the Quay API:

[source,bash,role=execute,subs="attributes"]
----
QUAY_ROUTE=$(oc get route -n {quay-namespace} -o jsonpath='{.items[0].spec.host}')
curl -sk "https://${QUAY_ROUTE}/api/v1/repository/{registry-user}/secure-nodejs/tag/latest/security?vulnerabilities=true" \
  | jq '{vulnerabilities: ([.data.Layer.Features[]?.Vulnerabilities[]? | select(. != null)] | length), status: .status}'
----
...
[NOTE]
====
If the scan shows `"status": "queued"`, wait 30-60 seconds and retry the query. Clair scanning happens asynchronously after the image is pushed to the registry.
====
...
echo "  cosign verify --key cosign.pub {workshop-registry}/{registry-user}/secure-nodejs:latest"
echo ""
echo "With keyless (Sigstore):"
echo "  cosign verify --certificate-identity=builder@example.com \\"
echo "    --certificate-oidc-issuer=https://token.actions.githubusercontent.com \\"
echo "    {workshop-registry}/{registry-user}/secure-nodejs:latest"
echo ""
...
----
# Pull and compare
podman pull {workshop-registry}/{registry-user}/secure-nodejs:latest 2>/dev/null || echo "Run on a machine with podman"

# Alternative: Check in Quay console
echo "Check image size at: {workshop-registry-console}/repository/{registry-user}/secure-nodejs"
----
