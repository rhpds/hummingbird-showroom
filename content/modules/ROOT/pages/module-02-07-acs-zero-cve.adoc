...
This policy blocks any deployment where the image contains fixable vulnerabilities. Create it via the ACS REST API:
...
[IMPORTANT]
====
**Enforcement Action**: Use `SCALE_TO_ZERO_ENFORCEMENT` (not `FAIL_DEPLOYMENT_CREATE_ENFORCEMENT`) for deploy-time policies. This is the enforcement type that the ACS admission controller webhook recognises for blocking deployments at admission time. With `SCALE_TO_ZERO_ENFORCEMENT`, the admission webhook will deny the Kubernetes API request before the pod is created.
====

[NOTE]
====
**UI alternative**: Navigate to *Platform Configuration* -> *Policy Management* -> *Create Policy*:
