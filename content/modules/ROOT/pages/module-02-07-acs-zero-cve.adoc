...
Click the link:https://{rhacs_route}[{rhacs_route}^] link to open the RHACS Central dashboard in a separate window.
...
curl -sk -H "Authorization: Bearer ${ROX_API_TOKEN}" \
  "https://${ACS_ROUTE}/v1/alerts?query=Namespace:{user}-hummingbird-acs-lab" | \
  python3 -c "
import json, sys
data = json.load(sys.stdin)
