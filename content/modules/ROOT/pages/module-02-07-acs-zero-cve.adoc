...
podman tag legacy-python-api:vulnerable \
  quay.io/{registry-user}/legacy-python-api:vulnerable

podman push quay.io/{registry-user}/legacy-python-api:vulnerable
----
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: api
        image: quay.io/REGISTRY_USER/legacy-python-api:vulnerable
        ports:
        - containerPort: 8080
        livenessProbe:
          httpGet:
...
[NOTE]
====
Replace `REGISTRY_USER` with your actual registry username, or use `sed` inline:

[source,bash,subs="attributes"]
----
oc set image deployment/legacy-python-api \
  api=quay.io/{registry-user}/legacy-python-api:vulnerable \
...
----
roxctl image scan \
  --image=quay.io/{registry-user}/legacy-python-api:vulnerable \
...
podman tag fortress-python-api:zero-cve \
  quay.io/{registry-user}/fortress-python-api:zero-cve

podman push quay.io/{registry-user}/fortress-python-api:zero-cve
----
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: api
        image: quay.io/REGISTRY_USER/fortress-python-api:zero-cve
        ports:
        - containerPort: 8080
        livenessProbe:
          httpGet:
...
oc set image deployment/fortress-python-api \
  api=quay.io/{registry-user}/fortress-python-api:zero-cve \
  -n hummingbird-acs-lab
...
----
roxctl image scan \
  --image=quay.io/{registry-user}/fortress-python-api:zero-cve \
  --insecure-skip-tls-verify \
  --output=table
----
