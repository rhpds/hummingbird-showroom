...
* `roxctl` CLI (installed in Step 2 below)
...
----
export WORKSHOP_REGISTRY=$(oc get route -n {quay-namespace} -o jsonpath='{.items[0].spec.host}')
export REGISTRY_USER=workshopuser
export REGISTRY_PASSWORD=workshoppass123

echo "WORKSHOP_REGISTRY=${WORKSHOP_REGISTRY}"
echo "REGISTRY_USER=${REGISTRY_USER}"

podman login -u "${REGISTRY_USER}" -p "${REGISTRY_PASSWORD}" "https://${WORKSHOP_REGISTRY}" --tls-verify=false
----

.*Expected output:*
----
WORKSHOP_REGISTRY=quay-registry-quay-quay.apps.<cluster-domain>
REGISTRY_USER=workshopuser
Login Succeeded!
----
...
[NOTE]
====
The download endpoint requires authentication. We use the ACS admin password stored in the `central-htpasswd` secret. Writing to `/usr/local/bin/` requires `sudo`.
====
...
[WARNING]
====
**If you open a new terminal** or your session disconnects, all `export` variables (`WORKSHOP_REGISTRY`, `REGISTRY_USER`, `REGISTRY_PASSWORD`, `ROX_ENDPOINT`, `ROX_API_TOKEN`) are lost. Re-run **Step 2** and **Step 4** before continuing. A quick way to tell: if `roxctl` complains about `localhost:8443`, it means `ROX_ENDPOINT` is not set.
====
...
podman tag legacy-python-api:vulnerable \
  ${WORKSHOP_REGISTRY}/${REGISTRY_USER}/legacy-python-api:vulnerable
...
The Quay repository is private, so OpenShift needs credentials to pull images:
...
podman tag fortress-python-api:zero-cve \
  ${WORKSHOP_REGISTRY}/${REGISTRY_USER}/fortress-python-api:zero-cve
...
.*Expected output (sizes will vary based on image versions):*
----
REPOSITORY:TAG                                   SIZE
localhost/legacy-python-api:vulnerable            ~900 MB
localhost/fortress-python-api:zero-cve            ~100 MB
----
...
Hummingbird images published by Red Hat through the Konflux pipeline include attached SBOM attestations, signatures, and SLSA provenance. Inspect the upstream image supply chain:
