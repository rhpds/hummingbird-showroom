...
----
oc login -u {user} -p {password} {openshift-api} --insecure-skip-tls-verify
----
...
[source,bash,role=execute,subs="attributes"]
----
oc new-project hummingbird-acs-lab-{user}
----

.*Expected output:*
----
Now using project "hummingbird-acs-lab-<your-user>" on server "https://...".
----
...
oc create secret docker-registry quay-pull-secret \
  ...
  -n hummingbird-acs-lab-{user} 2>/dev/null || echo "Secret already exists"

oc secrets link default quay-pull-secret --for=pull -n hummingbird-acs-lab-{user}
oc secrets link builder quay-pull-secret -n hummingbird-acs-lab-{user}
----
...
oc delete bc/legacy-python-api -n hummingbird-acs-lab-{user} 2>/dev/null || true
...
oc new-build --strategy=docker --binary=true \
  ...
  -n hummingbird-acs-lab-{user}

oc set build-secret --push bc/legacy-python-api quay-pull-secret -n hummingbird-acs-lab-{user} 2>/dev/null || true

cp Containerfile.legacy Dockerfile
oc start-build legacy-python-api --from-dir=. --follow --wait -n hummingbird-acs-lab-{user}
rm -f Dockerfile
----
...
metadata:
  name: legacy-python-api
  namespace: hummingbird-acs-lab-{user}
  labels:
...
[source,bash,role=execute,subs="attributes"]
----
oc rollout status deployment/legacy-python-api -n hummingbird-acs-lab-{user} --timeout=120s
----
...
[source,bash,role=execute,subs="attributes"]
----
LEGACY_ROUTE=$(oc get route legacy-python-api -n hummingbird-acs-lab-{user} -o jsonpath='{.spec.host}')
...
oc new-build --strategy=docker --binary=true \
  ...
  -n hummingbird-acs-lab-{user} &>/dev/null || true

oc set build-secret --push bc/fortress-python-api quay-pull-secret -n hummingbird-acs-lab-{user} 2>/dev/null || true

cp Containerfile.fortress Dockerfile
oc start-build fortress-python-api --from-dir=. --follow --wait -n hummingbird-acs-lab-{user}
rm -f Dockerfile
----
...
oc rollout status deployment/fortress-python-api -n hummingbird-acs-lab-{user} --timeout=120s
----
...
[source,bash,role=execute,subs="attributes"]
----
ROUTE=$(oc get route fortress-python-api -n hummingbird-acs-lab-{user} -o jsonpath='{.spec.host}')
...
export LAB_NAMESPACE="hummingbird-acs-lab-{user}"
...
Policy 1:
  ...
  - **Policy Scope**: namespace `hummingbird-acs-lab-<your-user>`
...
Both policies are now active and will be evaluated by the admission controller on every deployment create or update in your `hummingbird-acs-lab-<your-user>` namespace.
...
oc set image deployment/fortress-python-api \
  ...
  -n hummingbird-acs-lab-{user} 2>&1
...
[source,bash,role=execute,subs="attributes"]
----
oc get deployment fortress-python-api -n hummingbird-acs-lab-{user}
...
curl -sk -H "Authorization: Bearer ${ROX_API_TOKEN}" \
  "https://${ACS_ROUTE}/v1/alerts?query=Namespace:hummingbird-acs-lab-{user}" | \
  python3 -c "
import json, sys
data = json.load(sys.stdin)
...
[source,bash,role=execute,subs="attributes"]
----
oc delete project hummingbird-acs-lab-{user}
...
oc get events -n hummingbird-acs-lab-{user} --sort-by=.metadata.creationTimestamp | tail -20
----
