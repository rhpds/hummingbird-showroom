...
oc new-build --strategy=docker --binary=true \
  ...
  -n hummingbird-acs-lab 2>/dev/null || true

oc set build-secret --push bc/legacy-python-api quay-pull-secret -n hummingbird-acs-lab

cp Containerfile.legacy Dockerfile
oc start-build legacy-python-api --from-dir=. --follow -n hummingbird-acs-lab
rm -f Dockerfile
----
...
oc set build-secret --push bc/fortress-python-api quay-pull-secret -n hummingbird-acs-lab

cp Containerfile.fortress Dockerfile
oc start-build fortress-python-api --from-dir=. --follow -n hummingbird-acs-lab
rm -f Dockerfile
----
...
LEGACY_LAYERS=$(skopeo inspect --tls-verify=false \
  docker://${WORKSHOP_REGISTRY}/${REGISTRY_USER}/legacy-python-api:vulnerable 2>/dev/null)
echo "$LEGACY_LAYERS" | python3 -c "
import json, sys
d = json.load(sys.stdin)
...
FORTRESS_LAYERS=$(skopeo inspect --tls-verify=false \
  docker://${WORKSHOP_REGISTRY}/${REGISTRY_USER}/fortress-python-api:zero-cve 2>/dev/null)
echo "$FORTRESS_LAYERS" | python3 -c "
import json, sys
d = json.load(sys.stdin)
layers = d.get('Layers', d.get('LayersData', []))
print(f'  Layers: {len(layers)}')
