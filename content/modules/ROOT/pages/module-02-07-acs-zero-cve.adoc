...
[TIP]
====
If the service account token creation fails, generate an API token using `roxctl` or directly via the ACS API:
...
# Generate API token via roxctl
export ROX_API_TOKEN=$(roxctl --insecure-skip-tls-verify \
  -e "${ACS_ROUTE}:443" \
  -p "${ACS_PASSWORD}" \
  central init-bundles generate workshop-token \
  --output-format json \
  | jq -r '.token')

echo "Generated ACS API Token: ${ROX_API_TOKEN}"
----

**Option 2: Using ACS API directly**
...
If no webhook exists, enable it via the ACS API:

[source,bash,role=execute,subs="attributes"]
----
# Get cluster ID
ACS_ROUTE=$(oc get route central -n {acs-namespace} -o jsonpath='{.spec.host}')
CLUSTER_ID=$(roxctl --insecure-skip-tls-verify central clusters list \
  | grep -i "$(oc config current-context | cut -d/ -f2)" \
  | awk '{print $1}')
...
curl -sk -X PATCH \
  ...
  -H "Content-Type: application/json" \
  "https://${ACS_ROUTE}/v1/cluster/${CLUSTER_ID}" \
  -d '{
...
echo "Admission controller webhook enabled for cluster ${CLUSTER_ID}"
----

[NOTE]
====
Wait 30-60 seconds for the webhook configuration to propagate to the OpenShift API server before testing policy enforcement.
====
...
Query the policy violations via roxctl:

[source,bash,role=execute]
----
# List recent violations
roxctl deployment check \
  --deployment=legacy-python-api \
  --namespace=hummingbird-acs-lab \
  --insecure-skip-tls-verify 2>/dev/null \
  || echo "Deployment blocked by ACS admission controller"
...
curl -sk -H "Authorization: Bearer ${ROX_API_TOKEN}" \
  ...
  | jq '.alerts[] | {policy: .policy.name, time: .time, violations: .violations[].message}'
----
...
----
{
  "policy": "Zero Fixable CVEs Required",
  "time": "2026-03-13T...",
  "violations": "Image has 127 fixable CVEs"
}
----

[TIP]
====
For visual inspection, you can optionally view violations in the ACS Central UI:

. Navigate to *Violations*
. Filter by namespace `hummingbird-acs-lab`
. Observe which policies blocked the deployment and the specific CVE details
====
