...
spec:
  ...
  - name: dockerfile
    value: "Containerfile"
  output:
    image: {workshop-registry}/{registry-user}/sample-quarkus:latest
    credentials:
...
* **Output**: Push to `{workshop-registry}/{registry-user}/sample-quarkus:latest`
...
[source,bash,role=execute]
----
oc get buildrun sample-quarkus-app-run-1 -n hummingbird-builds -w
----
...
[source,bash,role=execute]
----
BUILDRUN_POD=$(oc get buildrun sample-quarkus-app-run-1 -n hummingbird-builds -o jsonpath='{.status.taskRunName}')-pod
...
[source,bash,role=execute]
----
oc get buildrun sample-quarkus-app-run-1 -n hummingbird-builds -o jsonpath='{.status.output.digest}'
echo ""
----
...
TOKEN=$(curl -sk "https://${QUAY_ROUTE}/v2/auth?service=${QUAY_ROUTE}&scope=repository:{registry-user}/sample-quarkus:pull" \
  -u "{registry-user}:{registry-password}" | python3 -c 'import json,sys; print(json.load(sys.stdin)["token"])')
curl -sk -H "Authorization: Bearer $TOKEN" \
  "https://${QUAY_ROUTE}/v2/{registry-user}/sample-quarkus/tags/list" | python3 -m json.tool
----

.*Expected output:*
----
{
    "name": "workshopuser/sample-quarkus",
    "tags": [
        "latest"
    ]
}
...
[source,bash,role=execute,subs="attributes"]
----
skopeo inspect --tls-verify=false --creds {registry-user}:{registry-password} docker://{workshop-registry}/{registry-user}/sample-quarkus:latest \
  | python3 -c "
import json,sys
...
oc expose deployment sample-quarkus --port=8080 -n hummingbird-builds

oc expose service sample-quarkus -n hummingbird-builds
----
...
[source,bash,role=execute]
----
ROUTE=$(oc get route sample-quarkus -n hummingbird-builds -o jsonpath='{.spec.host}')
...
[source,bash]
----
oc get pods -l app=sample-quarkus -n hummingbird-builds
oc logs -l app=sample-quarkus -n hummingbird-builds
----
====
...
[source,bash,role=execute,subs="attributes"]
----
echo "Check image at: {workshop-registry-console}/repository/{registry-user}/sample-quarkus"
echo ""
echo "Check Quay Security Scanner results for CVE counts."
----
...
oc describe buildrun sample-quarkus-app-run-1 -n hummingbird-builds
----
...
oc delete buildrun sample-quarkus-app-run-1 -n hummingbird-builds
...
----
oc secrets link default registry-credentials --for=pull -n hummingbird-builds
oc delete pod -l app=sample-quarkus -n hummingbird-builds
----
...
oc get pods -l app=sample-quarkus -n hummingbird-builds
oc logs -l app=sample-quarkus -n hummingbird-builds
oc get route sample-quarkus -n hummingbird-builds
----
