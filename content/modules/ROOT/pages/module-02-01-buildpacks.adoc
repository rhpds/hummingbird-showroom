...
In case you are still logged into the VM from Module 1 exit the VM by typing `exit` followed by pressing the `Enter` key.

Don't worry if you exit one too many times, simply press `Enter` to reconnect to your terminal.

== Log In to the Cluster

Before running any commands, verify you are logged in to the OpenShift cluster. If the Showroom terminal is not already authenticated, run:
...
[source,bash,role=execute,subs=attributes]
----
oc project hummingbird-builds-{user}
----
...
[source,bash,role=execute,subs=attributes]
----
oc get buildrun sample-quarkus-app-run-1 -n hummingbird-builds-{user} -w
----
...
[WARNING]
====
*If the BuildRun shows `False` / `Failed`*, delete it and re-trigger the build:

[source,bash,role=execute,subs=attributes]
----
# Check what went wrong
oc describe buildrun sample-quarkus-app-run-1 -n hummingbird-builds-{user}

# Delete the failed BuildRun
oc delete buildrun sample-quarkus-app-run-1 -n hummingbird-builds-{user}

# Re-create it (same manifest as Step 5)
oc create -f - <<EOF
apiVersion: shipwright.io/v1beta1
kind: BuildRun
metadata:
...
* **Registry push error** -- registry credentials missing or expired. Check the Troubleshooting section at the end of this module.
* **BLOB_UPLOAD_INVALID** -- NooBaa storage is recovering. Run `oc get noobaa -n openshift-storage` and wait for `Ready` before retrying.
* **Pending indefinitely** -- Tekton pipelines operator not ready. Run `oc get pods -n openshift-pipelines` to verify.
====
...
[source,bash,role=execute,subs=attributes]
----
BUILDRUN_POD=$(oc get buildrun sample-quarkus-app-run-1 -n hummingbird-builds-{user} -o jsonpath='{.status.taskRunName}')-pod

oc logs $BUILDRUN_POD -n hummingbird-builds-{user} --all-containers
----
...
...
[1/2] STEP 8/8: RUN mvn package -DskipTests -Dmaven.repo.local=/build/.m2
...
[INFO] BUILD SUCCESS
...
[2/2] STEP 1/9: FROM registry.access.redhat.com/hi/openjdk:21-runtime
...
[2/2] STEP 7/9: USER 65532
[2/2] STEP 9/9: ENTRYPOINT ["java", "-jar", "quarkus-run.jar"]
...
[INFO] Pushing image ...
----
...
[source,bash,role=execute,subs=attributes]
----
oc get buildrun sample-quarkus-app-run-1 -n hummingbird-builds-{user} -o jsonpath='{.status.output.digest}'
echo ""
----
...
oc expose deployment sample-quarkus --port=8080 -n hummingbird-builds-{user}

oc expose service sample-quarkus -n hummingbird-builds-{user}
----
...
[source,bash,role=execute,subs=attributes]
----
ROUTE=$(oc get route sample-quarkus -n hummingbird-builds-{user} -o jsonpath='{.spec.host}')
...
.*Expected output:*
----
Application URL: http://sample-quarkus-hummingbird-builds-{user}.apps.<cluster-domain>/hello
{
...
++++
<div class="solve-button-placeholder" data-module="module-02-01"></div>
++++

++++
<div class="validate-button-placeholder" data-module="module-02-01"></div>
++++
...
oc delete secret registry-credentials -n hummingbird-builds-{user}
oc create secret docker-registry registry-credentials \
  --docker-server="{quay_hostname}" \
  --docker-username={quay_user} \
  --docker-password={quay_password} \
  -n hummingbird-builds-{user}
oc secrets link pipeline registry-credentials -n hummingbird-builds-{user}
----
...
oc get pods -l app=sample-quarkus -n hummingbird-builds-{user}
oc logs -l app=sample-quarkus -n hummingbird-builds-{user}
oc get route sample-quarkus -n hummingbird-builds-{user}
----
