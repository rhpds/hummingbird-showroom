...
[source,bash,role=execute,subs=attributes]
----
oc get secret quay-registry-credentials -n {user}-hummingbird-builds
...
----
NAME                        TYPE                             DATA   AGE
quay-registry-credentials   kubernetes.io/dockerconfigjson   1      95m

Image pull secrets:  quay-registry-credentials
Mountable secrets:   quay-registry-credentials
----
...
[source,bash,role=execute,subs=attributes]
----
oc secrets link default quay-registry-credentials --for=pull -n {user}-hummingbird-builds
----
...
[NOTE]
====
The `oc secrets link default quay-registry-credentials --for=pull` step is required because Quay creates repositories as **private** by default (`CREATE_PRIVATE_REPO_ON_PUSH: true`). Without linking the pull secret to the `default` ServiceAccount, pods will fail with `ImagePullBackOff`.
====
...
.*Expected output:*
----
Application URL: http://sample-quarkus-{user}-hummingbird-builds.apps.<cluster-domain>/hello
{
    "message": "Hello from Hummingbird!",
    "runtime": "Quarkus + JDK 21.0.11"
}
----
...
----
oc get secret quay-registry-credentials -n {user}-hummingbird-builds
oc describe sa pipeline -n {user}-hummingbird-builds | grep quay-registry-credentials

# Recreate secret if needed (use your on-cluster Quay route)
oc delete secret quay-registry-credentials -n {user}-hummingbird-builds
oc create secret docker-registry quay-registry-credentials \
  --docker-server="{quay_hostname}" \
  --docker-username={quay_user} \
  --docker-password={quay_password} \
  -n {user}-hummingbird-builds
oc secrets link pipeline quay-registry-credentials -n {user}-hummingbird-builds
----
