...
spec:
  ...
  - name: dockerfile
    value: "Containerfile"
  output:
    image: {quay_url}/{quay_user}/sample-quarkus:latest
    credentials:
...
skopeo inspect docker://${QUAY_HOSTNAME}/{quay_user}/sample-quarkus:latest 2>/dev/null \
  | python3 -c "
import json,sys
...
[source,bash,role=execute,subs="attributes"]
----
echo "Check image at: {quay_url}/repository/{quay_user}/sample-quarkus"
echo ""
