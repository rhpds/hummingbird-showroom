...
[source,bash,role=execute]
----
oc get buildrun sample-quarkus-app-run-1 -w
----
...
[source,bash,role=execute]
----
BUILDRUN_POD=$(oc get buildrun sample-quarkus-app-run-1 -o jsonpath='{.status.taskRunName}')-pod
...
[source,bash,role=execute]
----
oc get buildrun sample-quarkus-app-run-1 -o jsonpath='{.status.output.digest}'
echo ""
----
...
[source,bash,role=execute,subs="attributes"]
----
skopeo inspect docker://{workshop-registry}/{registry-user}/sample-quarkus:latest 2>/dev/null \
  | python3 -c "
import json,sys
...
for k,v in sorted(labels.items()):
    if 'hummingbird' in k.lower():
        print(f'Label {k}: {v}')
" \
  || echo "Note: skopeo inspect requires TLS trust for the on-cluster registry"
----
...
[source,bash,role=execute]
----
ROUTE=$(oc get route sample-quarkus -o jsonpath='{.spec.host}')
