...
Cosign signs and verifies images stored in OCI registries. Before we sign our lab images, let's look at how the official hardened images are handled. 
The "Verify Build" section of the image page provided you with the key used to sign all of the images. This allows you to cryptographically prove the image you pulled is the one we published.
...
cosign verify --insecure-ignore-tlog \
  ...
  {hummingbird-registry}/openjdk:21-runtime
----
----
Verification for quay.io/hummingbird/openjdk:21-runtime --
...
Since our image lives in local Podman storage, we'll push the image to the lab instance of Red Hat Quay for signing.
...
You can use `cosign` to create signatures as well as verify the existing signature of a container image.
